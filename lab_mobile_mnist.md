...
> Возьмите **последнюю цифру номера вашей зачётной книжки** — это и есть номер вашего варианта (0–9).  
>
> Каждый студент выполняет **базовое задание** (шаги 1–8, общие для всех) **и** индивидуальное задание своего варианта.  
> Индивидуальное задание сдаётся вместе с основной работой и оценивается отдельно (**+3 балла**).

| Последняя цифра зачётки | Вариант |
|:-----------------------:|---------|
| **0** | Сохранение истории (Room DB) |
| **1** | Режим реального времени (Live Preview) |
| **2** | Мультицифровое распознавание |
| **3** | Цветовые темы и кастомизация UI |
| **4** | Экспорт результатов в PDF |
| **5** | Виджет на рабочем столе |
| **6** | Голосовое озвучивание результата |
| **7** | Сравнение двух изображений |
| **8** | Встроенная рисовалка (DrawView) |
| **9** | Статистика и точность |

---

### Вариант 0 — Сохранение истории распознаваний

> **Для студентов, у которых последняя цифра зачётки: 0**

Реализовать экран истории: каждое успешное распознавание сохраняется в локальную базу данных **Room** с полями: дата/время, распознанная цифра, уверенность (%). Добавить кнопку «История» в главное меню, которая открывает `RecyclerView` со списком всех прошлых результатов в порядке убывания даты. Предусмотреть кнопку полной очистки истории с диалогом подтверждения.

**Технологии:** Room Database, RecyclerView, ViewModel, LiveData  
**Минимум записей для демонстрации:** 10

**Что сдаётся:**
- Скриншот экрана истории с ≥10 записями
- Исходный код сущности `RecognitionEntity` и `DAO`-интерфейса
- Письменный ответ: чем Room удобнее прямого использования SQLite в Android?

---

### Вариант 1 — Режим реального времени (Live Preview)

> **Для студентов, у которых последняя цифра зачётки: 1**

Реализовать непрерывное распознавание через `ImageAnalysis` use case из CameraX. Модель запускается **автоматически каждые 700 мс** без нажатия кнопки, результат обновляется на экране в реальном времени. Добавить анимированный индикатор (пульсирующий круг или прогресс-бар), сигнализирующий об обработке. Интервал между анализами регулируется через `SeekBar` (диапазон 300–2000 мс).

**Технологии:** CameraX `ImageAnalysis`, Kotlin Coroutines, `Handler`

**Что сдаётся:**
- Видеозапись (screen recording) работающего живого режима (≥15 секунд)
- Исходный код класса-анализатора (`Analyzer`)
- Письменный ответ: почему нельзя запускать инференс TFLite на главном (UI) потоке?

---

### Вариант 2 — Мультицифровое распознавание

> **Для студентов, у которых последняя цифра зачётки: 2**

Добавить режим фотографирования **нескольких цифр** на одном листе (например, число «42» или «108»). Приложение разбивает изображение на сегменты методом вертикальных проекций яркости, распознаёт каждую цифру отдельно и выводит итоговое число целиком.

**Ограничение:** цифры пишутся в одну строку, не касаясь друг друга.  
**Технологии:** Bitmap-обработка, проекционная сегментация, TFLite

**Что сдаётся:**
- Фото тестового листа с числом из ≥3 цифр и скриншот результата
- Исходный код функции сегментации изображения
- Письменный ответ: какие ограничения имеет проекционный метод сегментации?

---

### Вариант 3 — Цветовые темы и кастомизация UI

> **Для студентов, у которых последняя цифра зачётки: 3**

Добавить **3 цветовые темы** (светлая, тёмная, высококонтрастная), переключаемые через экран настроек (`Settings Activity`). Реализовать анимацию появления результата распознавания (`ObjectAnimator`: fade-in + scale). Добавить кастомный `View`-компонент, отображающий распознанную цифру крупным шрифтом с плавной анимацией смены значения.

**Технологии:** `SharedPreferences`, Material Themes, Custom View, `ObjectAnimator`

**Что сдаётся:**
- 3 скриншота приложения в каждой теме
- Исходный код кастомного `View`-компонента
- Письменный ответ: чем `ObjectAnimator` отличается от `ViewPropertyAnimator`?

---

### Вариант 4 — Экспорт результатов в PDF

> **Для студентов, у которых последняя цифра зачётки: 4**

После распознавания предоставить возможность сохранить **отчёт в PDF-файл**. Отчёт содержит: дату и время, уменьшенный превью сделанной фотографии, распознанную цифру, уверенность модели (%), таблицу вероятностей для всех 10 классов. Добавить кнопку «Поделиться» для отправки PDF через `ShareCompat`.

**Технологии:** Android `PdfDocument` API, `FileProvider`, `ShareCompat`  
